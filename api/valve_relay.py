...
from services.valve_relay_service import turn_on_relay, turn_off_relay, get_relay_status

# Create Blueprint
valve_relay_blueprint = Blueprint('valve_relay', __name__)

# API Endpoint: Turn relay on
@valve_relay_blueprint.route('/<int:relay_id>/on', methods=['POST'])
...
@valve_relay_blueprint.route('/<int:relay_id>/off', methods=['POST'])
...
@valve_relay_blueprint.route('/<int:relay_id>/status', methods=['GET'])
