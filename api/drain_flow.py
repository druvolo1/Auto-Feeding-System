...
from services.drain_flow_service import get_latest_flow_rate, reset_total

drain_flow_blueprint = Blueprint('drain_flow', __name__)

@drain_flow_blueprint.route('/latest', methods=['GET'])
def latest_flow():
    flow_value = get_latest_flow_rate()
    if flow_value is not None:
        return jsonify({'status': 'success', 'flow': flow_value}), 200
    return jsonify({'status': 'failure', 'message': 'No drain flow reading available'}), 404
