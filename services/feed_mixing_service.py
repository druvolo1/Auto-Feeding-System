    ...
    while True:
        if feeding_sequence_active:
            ...
            with current_app.config['plant_lock']:
                ...
                # Example: Get first plant's data; adjust logic for multiple plants
                if not plant_data:
                    eventlet.sleep(1)
                    continue
            ...
            # Start mixing: Turn on valves and pump
            ...
            while feeding_sequence_active and not stop_mixing_flag:
                # Update volumes (use your flow meter functions)
                ...
                if total_volume >= system_volume:
                    log_mixing_feedback(f"Total volume reached for {system_name}, stopping mixing", status='info', sio=sio)
                    break
