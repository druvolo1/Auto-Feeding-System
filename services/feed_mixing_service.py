        ...
        with app.app_context():  # Create application context
            if stop_feeding_flag:
                ...
                if not components_off:
                    ...
                    plant_ip = app.config.get('current_plant_ip', last_processed_plant)  # Use last_processed_plant if None
                    ...
                    log_feeding_feedback(f"Feed mixing stopped due to feeding sequence interruption, turned off pump and relays", plant_ip or 'unknown', 'info', socketio)
                    components_off = True
                    mixed = False
            ...
            reset_key = f"{plant_ip or 'unknown'}_{phase}"  # Use 'unknown' if plant_ip is None
            if plant_ip != last_processed_plant or phase != 'fill':
                ...
                if reset_key != last_logged_reset:  # Log only if not recently logged
                    log_extended_feedback(f"Reset mixing state for new plant {plant_ip or 'unknown'} or phase change to {phase}", plant_ip or 'unknown', 'debug', socketio)
            ...
            if phase == 'fill' and plant_ip and not mixed and use_feed and not mixing_completed:
                ...
                # Turn on feed relay
                if feed_relay and not control_local_relay(fresh_relay, 'on', socketio, plant_ip):
                ...
                while True:
                    ...
                    if stop_feeding_flag or phase != 'fill':
                        ...
                        if fresh_relay:
                            control_local_relay(fresh_relay, 'off', socketio, plant_ip)
                        if stop_feeding_flag:
                            log_feeding_feedback(f"Feed mixing interrupted for {plant_ip or 'unknown'}, turned off pump and relays", plant_ip or 'unknown', 'error', socketio)
                        else:
                            log_feeding_feedback(f"Fill phase completed for {plant_ip or 'unknown'}, turned off feed pump and relays", plant_ip or 'unknown', 'info', socketio)
                        components_off = True
                        mixed = False
            ...
            if mixed and phase != 'fill' and not components_off:
                ...
                if fresh_relay:
                    control_local_relay(fresh_relay, 'off', socketio, plant_ip)
                log_feeding_feedback(f"Fill phase ended unexpectedly for {plant_ip or 'unknown'}, turned off feed pump and relays", plant_ip or 'unknown', 'info', socketio)
                components_off = True
                mixed = False
                mixing_completed = True
                last_processed_plant = plant_ip
            elif mixed and phase != 'fill':
                log_feeding_feedback(f"Feed mixing cycle for {plant_ip or 'unknown'} already cleaned up", plant_ip or 'unknown', 'debug', socketio)
                mixed = False
                mixing_completed = True
                last_processed_plant = plant_ip
