...
def monitor_feed_mixing(sio=None):
    ...
    log_mixing_feedback("Monitor feed mixing thread started", status='info', sio=sio)  # Confirm thread is running
    while True:
        ...
        # Cleanup outside loop if needed, but since it's in the while True, it's handled in the finally of a higher level if necessary
        eventlet.sleep(1)